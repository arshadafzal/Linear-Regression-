#  Program to find Regression Co-efficients in Linear Regression using the Conjugate-Gradient Method.
...
t = theta
r = np.dot(np.transpose(x), (np.dot(x, theta) - y))
p = r
for i in range(n - 1):
    # Learning Parameter Alpha
    alpha = np.dot(np.transpose(p), r)/np.dot(np.transpose(p), (np.dot((np.matmul(np.transpose(x), x)), p)))
    # Update Theta
    theta = theta - alpha * p
    #  Residual Vector
    r = r - alpha * (np.dot((np.matmul(np.transpose(x), x)), p))
    # Mean-squared Error
    ...
    t = theta
    #  Conjugate Vector
    u = np.dot(np.transpose(r), (np.dot((np.matmul(np.transpose(x), x)), p)))
    v = np.dot(np.transpose(p), (np.dot((np.matmul(np.transpose(x), x)), p)))
    beta = - u/v
    p = r + beta * p
print(theta)
